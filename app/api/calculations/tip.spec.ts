...
import { type TipRuleId, tipRuleDescriptions } from '../types/rules';
import { calculateTip } from './tip';

// Passing in an unreal points value in order to test object identity
function makeTipMock(tip: string, joker = false, points = -1): Tip {
  const newTip: Tip = {
    id: -1,
    firestoreId: '',
    championshipId: -1,
...
describe(`Tippberechnung nach Regeln: ${tipRuleDescriptions[0]?.name}`, () => {
  ...
  test('berechnet die korrekten Punkte', () => {
    for (const match of mockData) {
      const result = Result.makeFrom(match.result);
      match.tips.forEach((t, ix) => {
        const tip = makeTipMock(t.tip, t.joker);
        const points = calculateTip(tip, result, ruleId);
        expect(points).toBe(expectedPoints[ix]);
      });
    }
  });
  /*
